...
object Day7 extends aoc.Aoc("aoc2023/input7.txt", identity):
  ...
  def detectWithJokers(card: String): HandType =
    if (card.count(_ == 'J') > 0) card.replace("J", "").groupMapReduce(identity)(_ => 1)(_ + _).values.toList.sorted match {
      ...
      case List(1, 1, 1 | 2)   => HandType.Three     // x joker, 1 pair
      case List(2, 2)          => HandType.FullHouse // 1 joker, 2 pairs
      case List(1, 1 | 2 | 3)  => HandType.Four      // x joker, 3 same
      case List(1 | 2 | 3 | 4) => HandType.Five      // x joker all the same
      case other               => sys.error(s"unexpected $other")
    }
    else detectStraight(card) // no jokers

  case class Hand(cards: String, bid: Int, handType: HandType, joker: Boolean = false) extends Ordered[Hand]:
    ...
    def strongestWithJoker(): Hand = this.copy(handType = detectWithJokers(cards), joker = true)
