...
object Day4 extends Aso("aso2020/input4.txt", identity) {
  ...
  def validFields(s: String): Boolean =
    s match {
  ...
  def parse(lines: List[String], fields: Set[String], passports: List[Set[String]]): List[Set[String]] =
    lines match {
      case Nil                          => passports :+ fields
      case head :: rest if head.isEmpty => parse(rest, Set.empty, passports :+ fields)
      case head :: rest =>
        parse(rest, head.split(' ').filter(validFields).map(_.split(':').head).toSet ++ fields, passports)
    }
